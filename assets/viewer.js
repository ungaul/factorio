! function (o) {
    ...
    function replaceItemAndFluidTags(text) {
        return text.replace(/\[(item|fluid|recipe)=(.*?)\]/g, function (match, type, name) {
            ...
            console.log("Map info", e),
                function (t, e) {
                    ...
                    for (const surface of e.surfaces) {
                        const mapInstance = new r(t, surface);
                        mapInstance.baseLayer._surfaceName = surface.surface_name;
                        s.push(mapInstance);

                        categories.planets.push({
                            layer: mapInstance.baseLayer,
                            name: surface.surface_name,
                            className: "planet",
                            category: "#planets"
                        });

                        d.set(mapInstance.surfaceInfo.surface_idx.toString(), mapInstance);
                        d.set(mapInstance.surfaceInfo.surface_name, mapInstance);
                    }

                    function addCategory(categoryId, maps) {
                        maps.forEach(item => {
                            let icon = categoryId === "planets" ? item.name : "cargo-pod";

                            let displayName = `<span class="${item.className}">
                                <img src="assets/icons/${icon}.png" width="16" height="16" style="vertical-align: middle; margin-right: 5px;" onerror="this.onerror=null;this.src='assets/icons/cargo-pod.png';">
                                ${item.name}
                            </span>`;

                            a.addBaseLayer(item.layer, displayName);
                        });
                    }
                    ...
                    ["planets", "platforms", "layers"].forEach(cat => addCategory(cat, categories[cat] = customSort(categories[cat])));

                    const h = o.layerGroup(),
                        c = o.layerGroup(),
                        m = o.layerGroup();
                    a.addOverlay(h, "<img class='layer-icon' src='assets/station.png'>"), a.addOverlay(c, "<img class='layer-icon' src='assets/tag.png'>"), a.addOverlay(m, "<img class='layer-icon' src='assets/debug.png'>");
                    const p = new Map;
                    ...
                    let minBounds = g.worldToLatLng(g.surfaceInfo.world_min.x, g.surfaceInfo.world_min.y);
                    let maxBounds = g.worldToLatLng(g.surfaceInfo.world_max.x, g.surfaceInfo.world_max.y);
                    let bounds = L.latLngBounds(minBounds, maxBounds);
                    f.setMaxBounds(bounds);
                    f.options.maxBoundsViscosity = 1.0;
                    let b = l(_.get("x"), 0),
                        x = l(_.get("y"), 0),
                        y = l(_.get("z"), 0);
                    ...
                    })), f.on("baselayerchange", (e) => {
                        const selectedLayer = e.layer;
                        const found = Array.from(d.values()).find(item => item.baseLayer._surfaceName === selectedLayer._surfaceName);
                        if (!found) {
                            console.log("unknown layer", selectedLayer._surfaceName);
                            return;
                        }
                        g = found;
                        u(g);
                        const params = new URLSearchParams(window.location.search);
                        params.set("s", g.surfaceInfo.surface_idx.toString());
                        history.replaceState(null, "", "?" + params.toString());
                    });
                    function updateActiveLabels() {
                        if (!g || !g.baseLayer || !g.baseLayer._surfaceName) return;
                        const currentSurface = g.baseLayer._surfaceName.toLowerCase();
                        document.querySelectorAll(".planet, .platform").forEach(item => {
                            if (item.textContent.trim().toLowerCase() === currentSurface) {
                    ...
                    setTimeout(updateActiveLabels, 100);
                    f.on("baselayerchange", () => {
                        updateActiveLabels();
                    });
                    const z = o => {
                        const t = f.getZoom(),
